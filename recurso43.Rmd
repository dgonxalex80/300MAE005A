...
```{r, echo=FALSE, out.width="100%", fig.align = "center"}
knitr::include_graphics("img/decisones.png")
```
...
# Definición de datos
x <- seq(-4, 4, 0.01)
y <- dt(x, df = 99)  # distribución t de Student con 99 grados de libertad

# Límites de la región de rechazo
alpha <- 0.05
limite_izq <- qt(alpha / 2, df = 99)
limite_der <- qt(1 - alpha / 2, df = 99)
...
ggplot(df, aes(x, y)) +
  ...
  geom_ribbon(data = subset(df, x <= limite_izq),
              aes(ymin = 0, ymax = y),
              fill = "red", alpha = 0.3) +
  geom_ribbon(data = subset(df, x >= limite_der),
              aes(ymin = 0, ymax = y),
              fill = "red", alpha = 0.3) +
  ggtitle(paste0("Región de rechazo : (-Inf, ", round(limite_izq, 6), ")   (", round(limite_der, 6), ", +Inf)")) +
  theme_minimal()
...
# Definición de datos para Chi-cuadrado
x <- seq(0, 30, 0.01)  # Valores de X para la curva Chi-cuadrado
y <- dchisq(x, df = 9)  # Densidad Chi-cuadrado con 9 grados de libertad

# Nivel de significancia
alpha <- 0.05

# Cálculo de límites de rechazo para dos colas
limite_izq <- qchisq(alpha/2, df = 9)
limite_der <- qchisq(1 - alpha/2, df = 9)
...
ggplot(df_plot, aes(x = x, y = y)) +
  geom_line(color = "purple") +   # Línea principal
  geom_ribbon(data = subset(df_plot, x <= limite_izq),
              aes(ymin = 0, ymax = y),
              fill = "purple", alpha = 0.3) +
  geom_ribbon(data = subset(df_plot, x >= limite_der),
              aes(ymin = 0, ymax = y),
              fill = "purple", alpha = 0.3) +
  ggtitle(paste0("Región de rechazo: (0; ", round(limite_izq, 2), ") y (", round(limite_der, 2), ", Inf)")) +
  theme_minimal()
...
# **Puebas de hipótesis sobre dos poblaciones**
...
En todos los casos el valor-p es inferior al valor de $\alpha = 0.05$, por tal razón se asume que los datos siguen una distribución normal.
...
<div class="box3 with-label">
<div class="label">Tipos de hipótesis</div>
...
knitr::include_graphics("img/tutorial401.png")
```
...
```{r, echo=FALSE, out.width="100%"}
knitr::include_graphics("img/tutorial402.png")
```
