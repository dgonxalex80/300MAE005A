...
library(lmtest)
x1=c(24.3,12.5,31.2,28,35.1,10.5,23.2,10,8.5,15.9,14.7,15)
y1=c(16.2,8.5,15,17,24.2,11.2,15,7.1,3.5,11.5,10.7,9.2)

y3=c(2.745, 2.700, 2.690, 2.680, 2.675, 2.670, 2.665, 2.660, 2.655, 2.655, 2.650, 2.650, 2.645, 2.635, 2.630, 2.625, 2.625, 2.620, 2.615, 2.615, 2.615, 2.615, 2.590, 2.590, 2.565)
x3=c(2.080, 2.045, 2.050, 2.005, 2.035, 2.035, 2.020, 2.005, 2.010, 2.000, 2.000, 2.005, 2.015, 1.990, 1.990, 1.995, 1.985, 1.970, 1.985, 1.990, 1.995, 1.990, 1.975, 1.995, 1.955)

x4=c(939,5796,289,283,138,2698,663,1069,5945,4159,1266,1481,4716)
y4=c(251,4690,124,294,138,1085,345,355,5753,1377,802,945,2327)

x5=c(24.3,12.5,31.2,28,35.1,10.5,23.2,10,8.5,15.9,14.7,15)
y5=c(16.2,8.5,15,17,24.2,11.2,15,7.1,3.5,11.5,10.7,9.2)

df1 = data.frame(x1=c(24.3,12.5,31.2,28,35.1,10.5,23.2,10,8.5,15.9,14.7,15),
                y1=c(16.2,8.5,15,17,24.2,11.2,15,7.1,3.5,11.5,10.7,9.2))

df3 = data.frame(y3=c(2.745, 2.700, 2.690, 2.680, 2.675, 2.670, 2.665, 2.660, 2.655, 2.655, 2.650, 2.650, 2.645, 2.635, 2.630, 2.625, 2.625, 2.620, 2.615, 2.615, 2.615, 2.615, 2.590, 2.590, 2.565),
                 x3=c(2.080, 2.045, 2.050, 2.005, 2.035, 2.035, 2.020, 2.005, 2.010, 2.000, 2.000, 2.005, 2.015, 1.990, 1.990, 1.995, 1.985, 1.970, 1.985, 1.990, 1.995, 1.990, 1.975, 1.995, 1.955))

df4 =data.frame( x4=c(939,5796,289,283,138,2698,663,1069,5945,4159,1266,1481,4716),
                 y4=c(251,4690,124,294,138,1085,345,355,5753,1377,802,945,2327))

df5 = data.frame(x5=c(24.3,12.5,31.2,28,35.1,10.5,23.2,10,8.5,15.9,14.7,15),
                 y5=c(16.2,8.5,15,17,24.2,11.2,15,7.1,3.5,11.5,10.7,9.2))
...
```{r, echo=FALSE, out.width="100%", fig.align = "center"}
 knitr::include_graphics("img/recursos5.png")
```
...
plot(x1,y1, xlab = "Ingresos", ylab = "Consumo",
...
plot(x3,y3, xlab = "peso inicial", ylab = "peso final", pch=21, bg="blue", las=1)
```
...
X=matrix(c(unos,x3),nrow=25) # matriz X 
	
# Estimador MCO - Enfoque matricial
b=solve(t(X)%*%X)%*%(t(X)%*%y3)
...
Continuamos con los datos de Ejemplo 1  
...
# Diagrama de dispersión X vs Y
		plot(y3,x3, main="Diagrama de dispersión",xlab="X:Peso inicial", 
...
# Estimación por MCO
		...
		summary(regresion)
		confint(regresion)
		plot(y3~x3, xlab = "Peso inicial", ylab = "Peso final", pch=19)
		abline(regresion)
```
...
# Diagnostico de normalidad
		...
		qqline(residuos)
		plot(y3,x3, xlab = "peso-F", ylab = "peso-b")
		abline(y3~x3)
```
...
# validación de supuestos del modelo
		...
		valores.ajustados <- fitted(regresion)
		plot(valores.ajustados, residuos)
		lmtest::dwtest(y3~x3, data = df3)
		lmtest::bgtest(y3~x3, data = df3)
		shapiro.test(residuos)
```
...
<div class="box1 with-label">
<div class="label">Transformaciones</div>
...
Queremos estimar un intervalo de confianza para el consumo de un hogar (Y)  que tenga unos ingresos (X) de $x_{o}=18$  

En el supongamos que deseamos estimar un intervalo de confianza para el consumo ($\mu_{Y|x_{o}}$) para hogares con ingresos de $x_{o} = 18$.
...
El paquete `paqueteDEG` si debe ser instalado cada vez que sea utilizado con el fin de incorporar nuevas actualizaciones
