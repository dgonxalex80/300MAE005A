...
# Vector de palabras
palabras <- c("población", "parámetro", "censo", "muestra", "muestreo",
              "muestreo probabilistico", "muestreo no probabilistico",
              "muestreo aleatorio simple", "muestreo aleatorio estratificado",
              "muestreo por conglomerados", "muestreo por juicio",
              "muestreo por conveniencia", "muestreo bola de nieve",
              "estimador puntual", "estimador insesgado",
              "estimador consistente", "estimador eficiente",
              "estimación por intervalos de confianza", "pruebas de hipótesis", "distribución normal", "distribución t-Student", "ditribución chi-cuadrado", "distribución F", "tamaño de muestra", "media muesrtal", "varianza muestral", "proporción muestral", "inferencia", "confianza", "nivel de significancia", "error tipo I", "error tipo II", "potencia de una prueba")


# Todas las palabras con frecuencia 1
frecuencias <- rep(1, length(palabras))

# Generar la nube en un dispositivo PNG grande (opcional, pero ayuda a que quepan más palabras)
png("nube_palabras.png", width = 1200, height = 800, res = 120)

wordcloud(
  words       = palabras,
  freq        = frecuencias,
  min.freq    = 1,
  max.words   = Inf,             # Mostrar todas las palabras
  scale       = c(3, 0.5),       # Ajusta la escala (prueba c(2, 0.2), etc.)
  random.order= FALSE,
  rot.per     = 0,               # Sin rotaciones para que quepan mejor
  random.color= TRUE,
  colors      = brewer.pal(8, "Dark2"),
  fixed.asp   = FALSE            # No fijar la relación de aspecto
)
...
<div class="box1 with-label">
<center>
![](img/distribucionesm.png){width=100%}
</center>
</div>
...
Para una distribución chi-cuadrado calcule :
...
* Calule $t_{0.025}$, cuando $v = 14$

```{r}
qt(0.025,10)
```

* Calule $t_{0.975}$, cuando $v = 10$
...
labels = c("n=1", "n=5", "n=10", "n=20", "n =30", "n=50", "n=100", "n=500", "n=1000")
...
# Definir contenedor de dos columnas
library(knitr)

kable(matrix(c(
  "![](img/animation_hist.gif){width=100%}",
  "![](img/animation_qqnorm.gif){width=100%}"
), ncol = 2), escape = FALSE, align = 'c')
...
kable(matrix(c(
  "![](img/animation2_hist.gif){width=100%}",
  "![](img/animation2_qqnorm.gif){width=100%}"
), ncol = 2), escape = FALSE, align = 'c')
